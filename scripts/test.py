...
def givePlayerStats(player,position):
    ...
    aopm_plus_scaler, aopm_plus_pca, aopm_plus_model, aopm_plus_expected_columns, aopm_plus_avg_pred, df = load_model_components(f"AOPM+_{position}")
    adpm_plus_scaler, adpm_plus_pca, adpm_plus_model, adpm_plus_expected_columns, adpm_plus_avg_pred, df = load_model_components(f"ADPM+_{position}")
    aepm_plus_scaler, aepm_plus_pca, aepm_plus_model, aepm_plus_expected_columns, aepm_plus_avg_pred, df = load_model_components(f"AEPM+_{position}")
    ...
    predicted_player_stats["AOPM+"] = format_stat(
        preprocess_and_predict(df, player_df, aopm_plus_scaler, aopm_plus_pca, aopm_plus_model, aopm_plus_expected_columns, aopm_plus_avg_pred, allow_negative=True, is_bpm=True)
    )

    predicted_player_stats["ADPM+"] = format_stat(
        preprocess_and_predict(df, player_df, adpm_plus_scaler, adpm_plus_pca, adpm_plus_model, adpm_plus_expected_columns, adpm_plus_avg_pred, allow_negative=True, is_bpm=True)
    )

    # Get percentile for APM
    aepm_plus_percentile = preprocess_and_predict(df, player_df, aepm_plus_scaler, aepm_plus_pca, aepm_plus_model, aepm_plus_expected_columns, aepm_plus_avg_pred, allow_negative=True, is_bpm=True)["percentile"]
    aepm_plus_color = preprocess_and_predict(df, player_df, aepm_plus_scaler, aepm_plus_pca, aepm_plus_model, aepm_plus_expected_columns, aepm_plus_avg_pred, allow_negative=True, is_bpm=True)["color"]
    predicted_player_stats["AEPM+"] = format_stat(
        {
            "prediction": predicted_player_stats["AOPM+"]["value"] + predicted_player_stats["ADPM+"]["value"],
            "percentile": aepm_plus_percentile,
            "color": aepm_plus_color
        }
    )
...
#print(givePlayerStats("https://onlinecollegebasketball.org/player/202447","SF"))
