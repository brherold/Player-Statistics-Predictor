...
def load_model_components(predYear,year,is_INT):
    if is_INT != 1:
        return joblib.load(f'RecruitSkillPredictorCol/Models-3/Col-{predYear}-{year}.pkl')
    else:
        return joblib.load(f'RecruitSkillPredictorCol/Models-INT/Col-{predYear}-{year}-INT.pkl') 
...
def get_country(soup):
    ...
    country_tag = soup.find_all("img")[1]

    country = country_tag.get('title')
...
def get_eval(soup):
    ...
    InsideDefense = {
        "Average": {"Will never be a good perimeter defender"},
        "Bad": {"Will never be a good interior defender",}
    }
    PerimeterDefense = {
        "Average": {"Will never be a good interior defender"},
