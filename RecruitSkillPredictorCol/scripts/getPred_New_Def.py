...
def CurSkill(soup, is_INT):
    ...
    # Base columns as a plain dict (no DataFrame yet)
    row = {
        'FR_SI': 0, 'SO_SI': 0, 'JR_SI': 0, 'SR_SI': 0, 'FR-SR-Dev': 0, 'Eval_txt': "",
        "IS_Eval": 0, "OS_Eval": 0, "RNG_Eval": 0, "Reb_Eval": 0, "Def_Eval": 0,
        "ID_Eval": 0, "PD_Eval": 0, "IQ_Eval": 0, "PA_Eval": 0, "HND_Eval": 0,
        "Spd_Eval": 0, "All_Around_Eval": 0, 'FR_IS': 0, 'FR_OS': 0, 'FR_Rng': 0,
        'FR_Fin': 0, 'FR_Reb': 0, 'FR_IDef': 0, 'FR_PDef': 0, 'FR_IQ': 0,
        'FR_Pass': 0, 'FR_Hnd': 0, 'FR_Drv': 0, 'FR_Spd': 0, 'FR_Sta': 0,
    }

    row.update(eval_scores)
    row['POT'] = potential

    # Map of skill_values key -> column suffix
    skill_map = [
        ('IS', 'IS'), ('OS', 'OS'), ('RNG', 'Rng'), ('FIN', 'Fin'), ('REB', 'Reb'),
        ('ID', 'IDef'), ('PD', 'PDef'), ('IQ', 'IQ'), ('PASS', 'Pass'),
        ('HND', 'Hnd'), ('DRV', 'Drv'), ('SPD', 'Spd'), ('STA', 'Sta'),
    ]

    # period_labels[i] = the column prefix to use for skill_values index i
    if is_INT == 1:
        period_labels = ['HS-4', 'HS-4.5', 'Col-1', 'Col-1.5', 'Col-2', 'Col-2.5',
                          'Col-3', 'Col-3.5', 'Col-4', 'Col-4.5']
    else:
        period_labels = ['HS-1', 'HS-1.5', 'HS-2', 'HS-2.5', 'HS-3', 'HS-3.5',
                          'HS-4', 'HS-4.5', 'Col-1', 'Col-1.5', 'Col-2', 'Col-2.5',
                          'Col-3', 'Col-3.5', 'Col-4', 'Col-4.5']

    n_periods = len(skill_values['SI'])
    half_year = 0

    for i, label in enumerate(period_labels):
        if i >= n_periods:
            break
        for src_key, suffix in skill_map:
            row[f'{label}_{suffix}'] = skill_values[src_key][i]
        # odd index (i=1,3,5,...) = the ".5" half-year periods
        half_year = 1 if (i % 2 == 1) else 0
...
def getPredictedSkills(playerID,predYear):
    ...
    save_path = os.path.join(folder, f'{playerID}-{predYear}-playerSkils-2052.pkl')
